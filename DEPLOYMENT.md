...
`pnpm build` создаёт полностью статический сайт в папке `out`. Для его публикации Node.js и `node_modules` не нужны. На хостинг загружается только содержимое `out`.
