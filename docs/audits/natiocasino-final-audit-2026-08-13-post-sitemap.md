...
- Дата: 13 августа 2026 года, повторный аудит после публикации sitemap/robots
...
Блокирующих ошибок и запрета индексации не обнаружено: все 4 опубликованные HTML-страницы отвечают `200`, разрешены robots.txt, включены в sitemap.xml, имеют self-canonical, а raw- и rendered-директивы не содержат `noindex`. Приёмку не позволяют завершить три существенные системные ошибки: bonus/promotions CTA ведут на польскую локализацию, шесть legal/help-ссылок footer ведут на общий affiliate landing вместо заявленных документов, а в опубликованном контенте остались обрезанная строка и серия пунктуационных склеек.
...
5. Затем выполнить рекомендации по rating formats, metadata и raster assets (`AUD-005`, `AUD-006`, `AUD-008`).
